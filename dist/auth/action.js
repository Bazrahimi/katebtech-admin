...
import { clearResetUid, clearVerifyCookies, readResetUid, readVerifyCookies, setResetUid, setVerifyCookies, } from "./cookies";
...
import { authSchema, forgotPasswordSchema, resetPasswordSchema, verifyCodeSchema, } from "@katebtech/admin/session";
export const auth = async (_prevState, formData) => {
 ...
 */
export const forgotPassword = async (_prevState, formData) => {
    const rawEmail = String(formData.get("email") ?? "");
...
export const resendCode = async () => {
    ...
    if (!ctx) {
        return {
            ok: false,
            message: "Verification session expired. Please try again or request a new code.",
        };
    }
...
export const verifyCode = async (_prev, formData) => {
    const parsed = verifyCodeSchema.safeParse({
        code: String(formData.get("code") ?? "").trim(),
    });
    if (!parsed.success) {
        return toActionErrors(parsed.error, "Please enter the verification code.");
    }
