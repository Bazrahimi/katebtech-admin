...
export const createPostActions = ({ postgresUrl, sessionEncodedKey, deleteAsset, }) => {
    ...
    const createPost = async (_prevState, formData) => {
        ...
        try {
            ...
            const message = postData.statusCode === POST_STATUS.PUBLISHED
    ...
    const updatePost = async (_prevState, formData) => {
        const session = await getSession(sessionEncodedKey);
        if (!session) {
            return {
                ok: false,
        ...
        try {
            ...
            const message = data.statusCode === POST_STATUS.PUBLISHED
    ...
    const featurePostAction = async (_prev, formData) => {
        const session = await getSession(sessionEncodedKey);
        if (!session) {
      ...
      `;
            if (rows.length === 0) {
                return postFailure("Not authorized to manage this post or post not found.");
            }
            const nowFeatured = rows[0].isFeatured === true;
            revalidatePath(allPostPath);
            return postSuccess(nowFeatured ? "Published to homepage." : "Removed from homepage.");
        }
        catch (err) {
    ...
    const publishPostAction = async (_prev, formData) => {
        const session = await getSession(sessionEncodedKey);
        if (!session) {
            return postFailure("You must be logged in.");
        }
      ...
      `;
            if (rows.length === 0) {
                return postFailure("Not authorized to publish this post or post not found.");
            }
            revalidatePath(allPostPath);
    ...
    const archivePostAction = async (_prev, formData) => {
        const session = await getSession(sessionEncodedKey);
        if (!session) {
            return postFailure("You must be logged in.");
        }
      ...
      `;
            if (rows.length === 0) {
                return postFailure("Not authorized to archive this post or post not found.");
            }
            revalidatePath(allPostPath);
    ...
    const deletePostAction = async (_prev, formData) => {
        const session = await getSession(sessionEncodedKey);
        if (!session) {
            return postFailure("You must be logged in.");
        }
